...
- Minimum 3 points required to close a shape (popup-main.js:58-61)
- Must close shape before calculating (popup-main.js:99-102)
- Shoelace formula works for simple polygons (no self-intersections)

### Extension Limitations

- No persistent storage currently implemented (though "storage" permission is declared)
- No multi-tab or cross-session state management
- Background service worker is empty

### File Organization

- All extension files are in the `extension/` directory
- Icons are in `extension/icons/` (icon.svg, icon16.png, icon48.png, icon128.png)
- `extension/components/` directory exists but currently empty
- The README.md references a different project (DevHelper Code Analysis) and doesn't match the actual implementation
